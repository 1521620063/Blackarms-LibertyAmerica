...
ABLAWeaponTestActor::ABLAWeaponTestActor()
{
    PrimaryActorTick.bCanEverTick = true;
}

void ABLAWeaponTestActor::BeginPlay()
{
    Super::BeginPlay();

    const FVector Base = GetActorLocation() + FVector(0.0f, 0.0f, 500.0f);
    Shooter = GetWorld()->SpawnActor<ABLAPlayerCharacter>(ABLAPlayerCharacter::StaticClass(), Base + FVector(-250.0f, 0.0f, 0.0f), FRotator::ZeroRotator);
    Target = GetWorld()->SpawnActor<ABLAPlayerCharacter>(ABLAPlayerCharacter::StaticClass(), Base + FVector(250.0f, 0.0f, 0.0f), FRotator(0.0f, 180.0f, 0.0f));
    if (!Shooter || !Target)
    {
        Fail(TEXT("spawn"));
        return;
    ...
    PrimaryData = NewObject<UBLAWeaponDataAsset>(this);
    PrimaryData->WeaponData.WeaponType = EBLA_WeaponType::PulseRifle;
    ...
    SecondaryData = NewObject<UBLAWeaponDataAsset>(this);
    SecondaryData->WeaponData.WeaponType = EBLA_WeaponType::ScatterGun;
    ...
    TertiaryData = NewObject<UBLAWeaponDataAsset>(this);
    TertiaryData->WeaponData.WeaponType = EBLA_WeaponType::EnergyPistol;
    ...
    if (!Shooter->WeaponComponent->EquipWeapon(ABLAWeaponBase::StaticClass(), PrimaryData, 0)
        || !Shooter->WeaponComponent->EquipWeapon(ABLAWeaponBase::StaticClass(), SecondaryData, 1)
        || !Shooter->WeaponComponent->EquipWeapon(ABLAWeaponBase::StaticClass(), TertiaryData, 2))
    {
        Fail(TEXT("equip"));
        return;
    }
    Shooter->HitFeedbackComponent->OnHitConfirmed.AddUniqueDynamic(this, &ABLAWeaponTestActor::HandleHitFeedback);

    FBLAWeaponData CalculationData = PrimaryData->WeaponData;
    CalculationData.RangeFalloff = 0.5f;
    CalculationData.MaxRange = 1000.0f;
    if (!FMath::IsNearlyEqual(ABLADamageResolver::CalculateFinalDamage(CalculationData, TEXT("Body"), 0.0f, 0.0f), 20.0f)
        || !FMath::IsNearlyEqual(ABLADamageResolver::CalculateFinalDamage(CalculationData, TEXT("WeakPoint"), 0.0f, 0.0f), 40.0f)
        || !FMath::IsNearlyEqual(ABLADamageResolver::CalculateFinalDamage(CalculationData, TEXT("Limb"), 0.0f, 0.0f), 15.0f)
        || !FMath::IsNearlyEqual(ABLADamageResolver::CalculateFinalDamage(CalculationData, TEXT("Body"), 1000.0f, 0.0f), 10.0f)
        || !FMath::IsNearlyEqual(ABLADamageResolver::CalculateFinalDamage(CalculationData, TEXT("Body"), 0.0f, 25.0f), 15.0f))
    {
...
void ABLAWeaponTestActor::Tick(float DeltaSeconds)
{
    ...
    else if (Stage == 1 && ElapsedSeconds >= 0.15f)
    {
        ...
        bool bKilled = false;
        if (!ABLADamageResolver::ResolveDamage(Target, PrimaryData->WeaponData, TEXT("Body"), 0.0f, Shooter, AppliedDamage, bKilled)
...
void ABLAWeaponTestActor::HandleHitFeedback(float AppliedDamage, bool bKilled)
{
    ++FeedbackCount;
}
