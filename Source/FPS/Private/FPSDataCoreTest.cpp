...
AFPSDataCoreTest::AFPSDataCoreTest()
{
    PrimaryActorTick.bCanEverTick = false;
}

bool AFPSDataCoreTest::Require(bool bCondition, const TCHAR* Reason)
{
    if (bCondition)
    {
        return true;
    ...
    UE_LOG(LogTemp, Error, TEXT("FPS_DATACORE_FAILED reason=%s"), Reason);
    return false;
}

void AFPSDataCoreTest::BeginPlay()
{
    Super::BeginPlay();
    ...
    AFPSObjectiveManager* Manager = GetWorld()->SpawnActor<AFPSObjectiveManager>();
    AFPSDataCore* Core = GetWorld()->SpawnActor<AFPSDataCore>(TestCoreHome, FRotator::ZeroRotator);
    AFPSObjectiveZone* Zone = GetWorld()->SpawnActor<AFPSObjectiveZone>(TestZoneCenter, FRotator::ZeroRotator);
    AFPSPlayerCharacter* Attacker = GetWorld()->SpawnActor<AFPSPlayerCharacter>(TestAttackerStart, FRotator::ZeroRotator);
    AFPSPlayerCharacter* AttackerDummy = GetWorld()->SpawnActor<AFPSPlayerCharacter>(TestAttackerDummyStart, FRotator::ZeroRotator);
    AFPSBotCharacter* Defender = GetWorld()->SpawnActor<AFPSBotCharacter>(TestDefenderStart, FRotator::ZeroRotator);
    AFPSBotCharacter* DefenderDummy = GetWorld()->SpawnActor<AFPSBotCharacter>(TestDefenderDummyStart, FRotator::ZeroRotator);
    ...
    State->MatchMode = EFPS_MatchMode::DataCoreAttackDefense;
    Attacker->Team = EFPS_Team::Attackers;
    AttackerDummy->Team = EFPS_Team::Attackers;
    Defender->Team = EFPS_Team::Defenders;
    DefenderDummy->Team = EFPS_Team::Defenders;
    ...
    const auto PlaceAtZone = [&](AFPSCharacterBase* Combatant)
    {
        Combatant->SetActorLocation(TestZoneCenter);
    };

    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Available
        && State->CurrentObjectiveState == EFPS_ObjectiveState::Available, TEXT("reset_initial")))
    {
        return;
    }
    ...
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Carried && Core->IsCarriedBy(Attacker)
        && State->CurrentObjectiveState == EFPS_ObjectiveState::Carried, TEXT("pickup_state")))
    {
        return;
    }
    ...
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Dropped && Core->Carrier == nullptr,
    ...
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Carried, TEXT("repickup_state")))
    {
        return;
    }
    ...
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Planting
    ...
    Manager->Tick(0.1f);
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Carried
    ...
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Planted && Manager->IsPlanted(),
        TEXT("planted_state")))
    {
        return;
    }
    ...
    if (!Require(State->RoundPhase == EFPS_RoundPhase::Combat, TEXT("planted_round_continues")))
    {
        return;
    }

    Manager->Tick(0.1f);
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Uploading, TEXT("uploading_state")))
    {
        return;
    }
    ...
    Manager->Tick(1.0f);
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Defusing
    ...
    Defender->SetActorLocation(TestZoneCenter + FVector(600.0f, 0.0f, 0.0f));
    Manager->Tick(0.1f);
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Uploading
    ...
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Defused, TEXT("defused_state")))
    {
        return;
    }
    if (!Require(State->RoundPhase == EFPS_RoundPhase::RoundResult && Rounds->LastResult
        && Rounds->LastResult->Winner == EFPS_Team::Defenders
    ...
    if (!Require(State->DefendersScore == 1 && State->RoundPhase == EFPS_RoundPhase::Preparation,
    ...
    Manager->Tick(PlantSeconds + 0.5f);
    Manager->Tick(0.1f);
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Uploading, TEXT("upload_state")))
    {
        return;
    }
    Manager->Tick(UploadSeconds + 1.0f);
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Completed, TEXT("upload_completed")))
    {
        return;
    }
    if (!Require(State->RoundPhase == EFPS_RoundPhase::RoundResult && Rounds->LastResult
        && Rounds->LastResult->Winner == EFPS_Team::Attackers
    ...
    AFPSTacticalPoint* PlantPoint = GetWorld()->SpawnActor<AFPSTacticalPoint>(TestPlantPointLocation, FRotator::ZeroRotator);
    AFPSTacticalPoint* DefusePoint = GetWorld()->SpawnActor<AFPSTacticalPoint>(TestDefusePointLocation, FRotator::ZeroRotator);
    AFPSAIController* AttackerAI = GetWorld()->SpawnActor<AFPSAIController>();
    AFPSAIController* DefenderAI = GetWorld()->SpawnActor<AFPSAIController>();
    AFPSBotCharacter* AttackerBot = GetWorld()->SpawnActor<AFPSBotCharacter>(TestAttackerStart, FRotator::ZeroRotator);
    AFPSBotCharacter* DefenderBot = GetWorld()->SpawnActor<AFPSBotCharacter>(TestDefenderStart, FRotator::ZeroRotator);
    if (!Require(Tactics && PlantPoint && DefusePoint && AttackerAI && DefenderAI && AttackerBot && DefenderBot,
        TEXT("ai_spawn")))
    {
        return;
    }
    PlantPoint->PointType = EFPS_TacticalPointType::PlantPoint;
    PlantPoint->PreferredRole = EFPS_BotRole::Assault;
    PlantPoint->Priority = 3.0f;
    DefusePoint->PointType = EFPS_TacticalPointType::DefusePoint;
    DefusePoint->PreferredRole = EFPS_BotRole::Defender;
    DefusePoint->Priority = 3.0f;
    AttackerBot->Team = EFPS_Team::Attackers;
    DefenderBot->Team = EFPS_Team::Defenders;
    ...
    if (!Require(AttackerAI->ResolveObjectiveDirective(Manager, Tactics, nullptr)
        && Manager->ObjectiveState == EFPS_ObjectiveState::Carried, TEXT("ai_attacker_pickup")))
    {
        return;
    }
    if (!Require(DefenderAI->ResolveObjectiveDirective(Manager, Tactics, nullptr)
    ...
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Dropped, TEXT("ai_core_dropped")))
    {
        return;
    }
    if (!Require(DefenderAI->ResolveObjectiveDirective(Manager, Tactics, nullptr)
    ...
    AttackerBot->SetActorLocation(Core->GetActorLocation() + FVector(60.0f, 0.0f, 0.0f));
    if (!Require(AttackerAI->ResolveObjectiveDirective(Manager, Tactics, nullptr)
        && Manager->ObjectiveState == EFPS_ObjectiveState::Carried, TEXT("ai_attacker_repickup")))
    {
        return;
    }
    ...
    if (!Require(AttackerAI->ResolveObjectiveDirective(Manager, Tactics, nullptr)
        && Manager->ObjectiveState == EFPS_ObjectiveState::Planting
    ...
    Manager->Tick(PlantSeconds + 0.5f);
    Manager->Tick(0.1f);
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Uploading, TEXT("ai_planted_uploading")))
    {
        return;
    }
    if (!Require(AttackerAI->ResolveObjectiveDirective(Manager, Tactics, nullptr)
    ...
    if (!Require(DefenderAI->ResolveObjectiveDirective(Manager, Tactics, nullptr)
        && Manager->ObjectiveState == EFPS_ObjectiveState::Defusing, TEXT("ai_defender_defuse")))
    {
        return;
    }
    const int32 DefendersScoreBeforeAI = State->DefendersScore;
    Manager->Tick(DefuseSeconds + 0.2f);
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Defused
        && State->DefendersScore == DefendersScoreBeforeAI + 1
        && State->RoundPhase == EFPS_RoundPhase::RoundResult, TEXT("ai_defuse_round_result")))
    {
        return;
    }
    ...
    Manager->Tick(0.0f);
    Rounds->StartCombatPhase();
    for (AFPSCharacterBase* DefenderMember : Teams->GetTeamMembers(EFPS_Team::Defenders))
    {
        DefenderMember->HealthComponent->ApplyDamage(1000.0f, TEXT("Body"), Attacker);
    }
    if (!Require(State->RoundPhase == EFPS_RoundPhase::MatchResult && Rounds->LastResult
        && Rounds->LastResult->Winner == EFPS_Team::Attackers
    ...
    if (!Require(State->AttackersScore == 3 && State->DefendersScore == DefendersScoreBeforeAI + 1
        && Rounds->EndRound(EFPS_Team::Defenders, TEXT("Duplicate")) == false,
    ...
    Manager->Tick(0.1f);
    if (!Require(Manager->ObjectiveState == EFPS_ObjectiveState::Available && Manager->bCoreRecovered
    ...
    bTestSucceeded = true;
    UE_LOG(LogTemp, Display, TEXT("FPS_DATACORE_OK pickup=attacker_only drop=carrier_death repickup=1 plant_interrupt=movement_zone_damage plant=1 defuse_interrupt=zone_damage defuse=1 upload=1 timeout=1 elimination=1 reset=idempotent recovery=outside_area authority=manager"));
}
