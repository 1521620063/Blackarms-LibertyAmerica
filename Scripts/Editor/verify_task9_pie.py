...
def finish(success, message):
    ...
    log(f"FPS_TASK9_PIE_DRIVER_{'OK' if success else 'FAILED'} {message}")
    state["ending"] = True
...
def tick(_):
    ...
    tests = unreal.GameplayStatics.get_all_actors_of_class(game_world, unreal.FPSDataCoreTest)
    if len(tests) != 1:
...
unreal.log("FPS_TASK9_PIE_DRIVER_STARTED")
